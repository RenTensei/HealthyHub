...
const DataList = ({ lastMonth, water, calories, weight }) => {
  const averageCalories = Math.round(calories.map(({value}) => value).reduce((partialSum, a) => partialSum + a, 0) / calories.length);
  const averageWater = Math.round(water.map(({value}) => value).reduce((partialSum, a) => partialSum + a, 0) / water.length);
  const averageWeight = Math.round(weight.map(({value}) => value).reduce((partialSum, a) => partialSum + a, 0) / weight.length);
          ...
          <div className={styles.weightField}>
            {weight.map((item, index) => (
              <div key={index} className={styles.weightData}>
                <p className={lastMonth ? styles.dataWeightItemMonth : styles.dataWeightItemYear}>{item.value}</p>
...
DataList.propTypes = {
  ...
  water: PropTypes.arrayOf(PropTypes.exact({
  ...
  calories: PropTypes.arrayOf(PropTypes.exact({
    period: PropTypes.oneOfType([
      PropTypes.string.isRequired,
      PropTypes.number.isRequired,
    ]),
  ...
  weight: PropTypes.arrayOf(PropTypes.exact({
    period: PropTypes.oneOfType([
      PropTypes.string.isRequired,
      PropTypes.number.isRequired,
    ]),
